...
func (c *DBclient) SaveReportHistory(history *ReportHistory) error {
	query := `
        ...
        (project_name, datasource, report_url, created_at, max_value, min_value, average, total_count, alert_count, critical_count, warning_count) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `
...
func (c *DBclient) GetReportHistory(limit, offset int) ([]ReportHistory, error) {
	query := `
        SELECT id, project_name, datasource, report_url, created_at, max_value, min_value, average, total_count, alert_count, critical_count, warning_count
